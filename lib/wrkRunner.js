    ...
    run() {
        return Promise
            ...
            .spread((upstreamIpAddress, proxyIpAddress, wrkIpAddress) => {
                ...
                const numBenchmarks = 2 * config.delays.length * config.connections.length * config.messageLengths.length * (config.dockerImages.proxies.length + 1);
                            ...
                            return Promise.each(config.messageLengths, messageLength => {
                                ...
                                return Promise.each(images, (image, i) => {
                                    ...
                                    if (image === "direct") {
                                        ...
                                        url = `http://${proxyIpAddress}:${80 + i}/${messageLength}?delay=${delay}`;
                                    }
