...
module.exports = {
    ...
    run() {
        return Promise
            ...
            .spread((upstreamIpAddress, proxyIpAddress, wrkIpAddress) => {
                ...
                const numBenchmarks = config.requestRates.length * config.compressions.length * config.ssls.length * config.delays.length * config.connections.length * config.messageLengths.length * (config.dockerImages.proxies.length + 1);
                                ...
                                return Promise.each(config.messageLengths, messageLength => {
                                    return Promise.each(config.requestRates, requestRate => {
                                        ...
                                        return Promise.each(images, (image, i) => {
                                            ...
                                            let threads = Math.ceil(connections * requestRate / config.connectionsPerThreadRequest);

                                            // wrk seems to die if thread count exceeds request rate
                                            if (threads > requestRate) threads = requestRate;
                                            ...
                                            return Promise.resolve()
                                                ...
                                                .then(results => this._writeResultsToFile(delay, ssl, compression, connections, messageLength, requestRate, image, results))
                                                .then(() => {
     ...
     * @private
     */
    _writeResultsToFile(delay, ssl, compression, connections, messageLength, requestRate, image, results) {
        return Promise.resolve()
            ...
            .then(data => {
                ...
                const i = _.findIndex(allResults, { delay, ssl, compression, connections, messageLength, requestRate, image });
                if (i >= 0) {
                    allResults[i] = entry;
                } else {
