    ...
    run() {
        return Promise
            ...
            .spread((upstreamIpAddress, proxyIpAddress, wrkIpAddress) => {
                ...
                const numBenchmarks = config.compressions.length * config.keepAlives.length * config.ssls.length * config.delays.length * config.connections.length * config.messageLengths.length * (config.dockerImages.proxies.length + 1);
