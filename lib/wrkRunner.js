...
module.exports = {
    ...
    run() {
        return Promise
            ...
            .spread((upstreamIpAddress, proxyIpAddress, wrkIpAddress) => {
                ...
                const numBenchmarks = 2 * 2 * 2 * config.delays.length * config.connections.length * config.messageLengths.length * (config.dockerImages.proxies.length + 1);
                ...
                return Promise.each([true, false], compression => {
                    return Promise.each([true, false], keepAlive => {
                        return Promise.each([true, false], ssl => {
                                    ...
                                    return Promise.each(config.messageLengths, messageLength => {
                                        ...
                                        return Promise.each(images, (image, i) => {
                                            ...
                                            return Promise.resolve()
                                                ...
                                                .then(results => this._writeResultsToFile(delay, keepAlive, ssl, compression, connections, messageLength, image, results))
                                                .then(() => {
     ...
     * @private
     */
    _writeResultsToFile(delay, keepAlive, ssl, compression, connections, messageLength, image, results) {
        return Promise.resolve()
            ...
            .then(data => {
                ...
                const i = _.findIndex(allResults, { delay, keepAlive, ssl, compression, connections, messageLength, image });
                if (i >= 0) {
                    allResults[i] = entry;
                } else {
