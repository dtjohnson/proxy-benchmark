...
module.exports = {
    ...
    run() {
        return Promise
            ...
            .spread((upstreamIpAddress, proxyIpAddress, wrkIpAddress) => {
                ...
                const numBenchmarks = 2 * config.connections.length * config.messageLengths.length * (config.dockerImages.proxies.length + 1);
                    ...
                    return Promise.each(config.connections, connections => {
                        return Promise.each(config.messageLengths, messageLength => {
                            ...
                            return Promise.each(images, (image, i) => {
                                ...
                                let url;
                                if (image === "direct") {
                                    url = `http://${upstreamIpAddress}/${messageLength}`;
                                } else {
                                    url = `http://${proxyIpAddress}:${80 + i}/${messageLength}`;
                                }
                                ...
                                return Promise.resolve()
                                    ...
                                    .then(results => this._writeResultsToFile(compression, connections, messageLength, image, results))
                                    .then(() => {
     ...
     * @private
     */
    _writeResultsToFile(compression, connections, messageLength, image, results) {
        return Promise.resolve()
            ...
            .then(data => {
                ...
                const i = _.findIndex(allResults, { compression, connections, messageLength, image });
                if (i >= 0) {
                    allResults[i] = entry;
                } else {
